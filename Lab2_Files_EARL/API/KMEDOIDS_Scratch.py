...
class Kmedoids:
    ...
    def fit(self,X):
        ...
        self.medoids = X[np.random.choice(X.shape[0], self.clusters, replace=False)]
        ...
        for i in range(self.max_iter):
            ...
            for i in range(self.clusters):
                ...
                cost = np.sum(np.linalg.norm(cluster_points - cluster_points[:, np.newaxis], axis=2), axis=1)
